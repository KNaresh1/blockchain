		...
		describe('filling orders', async () => {
			...
			describe('success', async () => {
				...
				it('executes the trade and charges fees', async () => {
					...
					balance.toString().should.equal(tokens(1).toString(), 'user1 received 1 token')
					balance = await exchange.balanceOf(ETHER_ADDRESS, user2)
					balance.toString().should.equal(ether(1).toString(), 'user2 received 1 ether')
					balance = await exchange.balanceOf(ETHER_ADDRESS, user1)
					balance.toString().should.equal('0', 'user1 deducted with 1 ether')
				...
				it('emits a "Trade" event', async () => {
					...
					event.tokenGive.should.equal(ETHER_ADDRESS, 'tokenGive is correct')
					event.amountGive.toString().shoud.equal(ether(1).toString(), 'amountGive is correct')
